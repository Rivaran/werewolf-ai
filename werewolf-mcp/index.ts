...
server.tool(
  ...
  async ({ player_number }) => {
    ...
    if (!state) return { content: [{ type: "text", text: "ゲームが開始されていません。" }] }
    const player = state.players[player_number - 1]
    if (!player) return { content: [{ type: "text", text: `プレイヤー${player_number}は存在しません。` }] }
...
server.tool(
  ...
  async ({ player_number, message }) => {
    const state = await loadState()
    if (!state) return { content: [{ type: "text", text: "ゲームが開始されていません。" }] }
    if (!state.gameId) return { content: [{ type: "text", text: "ゲームIDが見つかりません。AIモードでゲームを開始してください。" }] }

    const characterId = state.playerAssignments?.[player_number]
    if (!characterId) return { content: [{ type: "text", text: `プレイヤー${player_number}のキャラクター割り当てがありません。` }] }
    ...
    if (SUPABASE_URL && SUPABASE_KEY) {
      ...
      if (error) return { content: [{ type: "text", text: `エラー: ${error.message}` }] }
    } else {
      ...
      if (!res.ok) return { content: [{ type: "text", text: "投稿に失敗しました。" }] }
    }
